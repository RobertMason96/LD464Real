...
public class RobertSnakeMove : MonoBehaviour
{
    ...
    private GameObject findFood()
    {
        ...
        if (F.Length != 0)
        {
            ...
            foreach (GameObject gObj in F) 
            {
                if((Mathf.Abs(DeBody[0].transform.position.x- gObj.transform.position.x)<gridsize/2)||(Mathf.Abs(DeBody[0].transform.position.z - gObj.transform.position.z)<gridsize/2))
                {
